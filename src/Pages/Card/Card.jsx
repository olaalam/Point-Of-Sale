...
export default function Card({
  ...
  const { subTotal, totalTax, totalOtherCharge, totalAmountDisplay } = useMemo(() => {
    const calculatedSubTotal = Array.isArray(orderItems) ? orderItems.reduce((acc, item) => {
  ...
  const doneItems = useMemo(() => {
    return Array.isArray(orderItems) ? orderItems.filter((item) => item.preparation_status === "done") : [];
  }, [orderItems]);

  // Memoize items and total amount for checkout
  const { checkoutItems, amountToPay } = useMemo(() => {
    if (orderType === "dine_in") {
      const itemsToPayFor = doneItems.filter((item) => selectedPaymentItems.includes(item.temp_id));
      const selectedSubTotal = itemsToPayFor.reduce((acc, item) => acc + (item.price || 0) * (item.count || 1), 0);
  ...
  }, [orderItems, orderType, totalAmountDisplay, selectedPaymentItems, doneItems]);
  ...
  const currentLowestSelectedStatus = useMemo(() => {
    ...
    const selectedItemStatuses = orderItems.filter(item => selectedItems.includes(item.temp_id)).map(item => item.preparation_status || "pending");
  ...
  const handleUpdatePreparationStatus = async (itemTempId) => {
    ...
    const nextStatus = PREPARATION_STATUSES[itemToUpdate.preparation_status || "pending"]?.nextStatus;
    ...
    formData.append("preparing[0][status]", PREPARATION_STATUSES[nextStatus]?.apiValue || nextStatus);
    try {
      await postData("cashier/preparing", formData);
      const updatedItems = orderItems.map(item => item.temp_id === itemTempId ? { ...item, preparation_status: nextStatus } : item);
      updateOrderItems(updatedItems);
      toast.success("Status updated successfully!");
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.response?.data?.exception || "Failed to update status.";
      toast.error(errorMessage);
    } finally {
  ...
  const confirmVoidItem = async () => {
    ...
    const formData = new FormData();
    const cartIds = Array.isArray(itemToVoid.cart_id) ? itemToVoid.cart_id : (typeof itemToVoid.cart_id === "string" ? itemToVoid.cart_id.split(",").map(id => id.trim()) : [itemToVoid.cart_id]);
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.response?.data?.exception || "Failed to void item.";
      toast.error(errorMessage);
    } finally {
  ...
  const handleIncrease = (itemTempId) => {
    const updatedItems = orderItems.map(item => item.temp_id === itemTempId ? { ...item, count: (item.count || 0) + 1 } : item);
    updateOrderItems(updatedItems);
  };
  ...
  const handleDecrease = (itemTempId) => {
    const updatedItems = orderItems.map(item => {
      if (item.temp_id === itemTempId) {
        ...
        return newCount > 0 || item.preparation_status === "done" ? { ...item, count: Math.max(newCount, 1) } : null;
      }
      return item;
    }).filter(Boolean);
    updateOrderItems(updatedItems);
  ...
  const toggleSelectPaymentItem = (tempId) => {
    setSelectedPaymentItems(prev => prev.includes(tempId) ? prev.filter(id => id !== tempId) : [...prev, tempId]);
  };

  const handleSelectAll = () => {
    const allTempIds = orderItems.map(item => item.temp_id);
    setSelectedItems(prev => prev.length === allTempIds.length ? [] : allTempIds);
  };

  const handleSelectAllPaymentItems = () => {
    const allDoneTempIds = doneItems.map(item => item.temp_id);
    setSelectedPaymentItems(prev => prev.length === allDoneTempIds.length ? [] : allDoneTempIds);
  };

  const applyBulkStatus = async () => {
    if (!bulkStatus || selectedItems.length === 0 || !tableId) {
      toast.warning("Please select items, choose a status, and ensure a Table ID is set.");
      return;
    }
    const itemsToUpdate = orderItems.filter(item => selectedItems.includes(item.temp_id));
    const itemsForApi = itemsToUpdate.filter(item => PREPARATION_STATUSES[bulkStatus]?.canSendToAPI && item.cart_id);
    if (itemsForApi.length > 0) {
      const formData = new FormData();
      formData.append("table_id", tableId.toString());
      itemsForApi.forEach((item, index) => {
        formData.append(`preparing[${index}][cart_id]`, item.cart_id.toString());
        formData.append(`preparing[${index}][status]`, PREPARATION_STATUSES[bulkStatus].apiValue);
      });
      try {
        await postData("cashier/preparing", formData);
        toast.success(`Successfully updated ${itemsForApi.length} items to ${PREPARATION_STATUSES[bulkStatus].label}`);
      } catch (err) {
        toast.error(err.response?.data?.message || "Failed to update status.");
        return;
      }
    } else if (!PREPARATION_STATUSES[bulkStatus]?.canSendToAPI) {
      toast.info(`Status updated to ${PREPARATION_STATUSES[bulkStatus]?.label} locally.`);
    } else {
      toast.warning("No valid items to update.");
      return;
    }
    const updatedItems = orderItems.map(item => selectedItems.includes(item.temp_id) ? { ...item, preparation_status: bulkStatus } : item);
    updateOrderItems(updatedItems);
    setSelectedItems([]);
    setBulkStatus("");
  };
    ...
    <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-start mb-4 gap-4 flex-wrap p-4 bg-white rounded-lg shadow-md">
            <Select value={bulkStatus} onValueChange={setBulkStatus}>
              ...
              <SelectContent className="bg-white border border-gray-200 rounded-md shadow-lg">
                {Object.entries(PREPARATION_STATUSES).filter(([key]) => statusOrder.indexOf(key) >= statusOrder.indexOf(currentLowestSelectedStatus)).map(([key, value]) => (
      ...
      <div className="flex-1 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        ...
        {showClearAllConfirm && (
          <div className="fixed inset-0 bg-gray-500/50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full mx-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Confirm Clear All Items</h3>
              <p className="text-gray-600 mb-6">Are you sure you want to remove all {orderItems?.length || 0} items? This action cannot be undone.</p>
              <div className="flex justify-end gap-3">
                <Button onClick={() => setShowClearAllConfirm(false)} variant="outline">Cancel</Button>
                <Button onClick={confirmClearAllItems} className="bg-red-600 text-white hover:bg-red-700">Clear All Items</Button>
              </div>
            </div>
          </div>
        )}
              ...
              <tr>
                {orderType === "dine_in" && (
                  <th className="py-3 px-4 text-center text-gray-600 font-semibold">
                    <input type="checkbox" checked={selectedItems.length > 0 && selectedItems.length === orderItems.length} onChange={handleSelectAll} />
                  </th>
                )}
                <th className="py-3 px-4 text-center text-gray-600 font-semibold">Item</th>
                <th className="py-3 px-4 text-center text-gray-600 font-semibold">Price</th>
                <th className="py-3 px-4 text-center text-gray-600 font-semibold">Quantity</th>
                {orderType === "dine_in" && (<th className="py-3 px-4 text-center text-gray-600 font-semibold">Preparation</th>)}
                {orderType === "dine_in" && (<th className="py-3 px-4 text-center text-gray-600 font-semibold">Pay</th>)}
                <th className="py-3 px-4 text-right text-gray-600 font-semibold">Total</th>
                <th className="py-3 px-4 text-right text-gray-600 font-semibold">Void</th>
              </tr>
            </thead>
            <tbody>
              {orderItems.length === 0 ? (
                <tr>
                  <td colSpan={orderType === "dine_in" ? 8 : 6} className="text-center py-4 text-gray-500">No items found for this order.</td>
                </tr>
              ) : (
        ...
        {orderType === "dine_in" && doneItems.length > 0 && (
          <div className="mt-6">
            <DoneItemsSection doneItems={doneItems} selectedPaymentItems={selectedPaymentItems} toggleSelectPaymentItem={toggleSelectPaymentItem} handleSelectAllPaymentItems={handleSelectAllPaymentItems} />
          </div>
        )}
      </div>
      ...
      <div className="flex-shrink-0 bg-white border-t-2 border-gray-200 pt-6 mt-4">
        ...
        {orderType === "dine_in" && (
          <>
            <div className="grid grid-cols-2 gap-4 items-center mb-4">
              ...
              <p className="text-right text-lg font-semibold">{totalAmountDisplay.toFixed(2)} EGP</p>
            </div>
            <div className="grid grid-cols-2 gap-4 items-center mb-4">
              <p className="text-gray-600">Selected Items ({selectedPaymentItems.length}):</p>
              <p className="text-right text-lg font-semibold text-green-600">{amountToPay.toFixed(2)} EGP</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 items-center mb-6">
          ...
          <p className="text-right text-2xl font-bold text-green-700">{amountToPay.toFixed(2)} EGP</p>
        </div>
        <div className="flex justify-center gap-4">
          <Button onClick={handleViewOrders} className="bg-gray-500 text-white hover:bg-gray-600 text-lg px-8 py-3" disabled={isLoading}>View Orders</Button>
          <Button onClick={handleCheckOut} className="bg-bg-primary text-white hover:bg-red-700 text-lg px-8 py-3" disabled={isLoading || orderItems.length === 0 || (orderType === "dine_in" && selectedPaymentItems.length === 0)}>Checkout</Button>
        </div>
      </div>
      <VoidItemModal
        open={showVoidModal}
