import i18n from "i18next";
import { Save } from "lucide-react";
import { initReactI18next } from "react-i18next";

const resources = {
  en: {
    translation: {
      title: "Food2go",
      profile: "Profile",
      dueUsers: "Due Users",
      closeshift: "Close Shift",
      Profile: "Profile",
      exit: "Exit",
      shift: "Shift",
      //home
      Failedtofetchdiscountdata: "Failed to fetch discount data.",
      IncompletetransferdataCannotcompletetransfer: "Incomplete transfer data. Cannot complete transfer.",
      OrderTransferred: "Order transferred successfully from table {{sourceTableId}} to table {{newTableId}}.",
      FailedtocompletetransferPleasetryagain: "Failed to complete transfer. Please try again.",
      take_away: "Take Away",
      Delivery: "Delivery",
      Dinein: "Dine In",
      //navbar
      YouMustCloseShiftBeforeLogout: "You must close the shift before logging out.",
      LoggedOutSuccessfully: "Logged out successfully",
      Errorwhileloggingout: "Error while logging out",
      code: "Code",


      //card
      Allitemsclearedfromtheorder: "All items cleared from the order.",
      Noitemstosaveaspending: "No items to save as pending.",
      Nospecialinstructions: "No special instructions",
      Ordersavedaspending: "Order saved as pending!",
      Failedtosaveaspending: "Failed to save as pending.",
      Pleaseenteranoffercode: "Please enter an offer code.",
      OffervalidatedsuccessfullyPleaseconfirm: "Offer validated successfully! Please confirm.",
      Offerdetailsareincompleteintheresponse: "Offer details are in complete in the response",
      FailedtovalidateofferPleasetryagain: "Failed to validate offer. Please try again.",
      RewardAdded: "Reward item \"{{product}}\" successfully added to the order",
      SelectedItems: "Selected Items ({{count}}):",
      Failedtoapproveoffer: "Failed to approve offer.",
      Pleaseenteradealcode: "Please enter a deal code.",
      DealvalidatedsuccessfullyPleaseconfirm: "Deal validated successfully! Please confirm.",
      Unexpectedresponsefromserverorinvaliddeal: "Unexpected response from server or invalid deal",
      FailedtoapplydealPleasetryagain: "Failed to apply deal. Please try again.",
      DealAdded: "Deal \"{{deal_title}}\" successfully added to the order!",
      CannottransferorderTableIDorCartIDsaremissing: "Cannot transfer order: Table ID or Cart IDs are missing.",
      Pleaseselectanewtabletotransfertheorder: "Please select a new table to transfer the order.",
      Failedtoidentifytheitemtoupdate: "Failed to identify the item to update.",
      Missingrequireddatatoupdateitemstatus: "Missing required data to update item status.",
      StatuscannotbeupdatedviaAPIatthistime: "Status cannot be updated via API at this time.",
      Failedtoapplythestatusupdatelocally: "Failed to apply the status update locally.",
      Failedtoupdatestatus: "Failed to update status.",
      PleasefillinallrequiredfieldsManagerIDandPassword: "Please fill in all required fields: Manager ID and Password.",
      Itemvoidedsuccessfully: "Item voided successfully!",
      InvalidManagerIDorPasswordAccessdenied: "Invalid Manager ID or Password. Access denied.",
      NoresponsefromserverCheckyourinternetconnection: "No response from server. Check your internet connection.",
      Anunexpectederroroccurred: "An unexpected error occurred.",
      Pleaseselectitemstopayforfromthedoneitemslist: "Please select items to pay for from the done items list.",
      PleaseselectitemschooseastatusandensureaTableIDisset: "Please select items, choose a status, and ensure a Table ID is set.",
      Novaliditemstoupdate: "No valid items to update.",
      BulkUpdateSuccess: "Successfully updated {{count}} items to {{status}}",
      Itemremovedsuccessfully: "Item removed successfully",
      Noitemstoclear: "No items to clear.",
      OrderDetails: "Order Details",
      ClearAllItems: "Clear All Items",
      ViewOrders: "View Orders",
      ApplyOffer: "Apply Offer",
      ApplyDeal: "Apply Deal",
      ChangeTable: "Change Table",
      ApplyStatus: "Apply Status",
      ConfirmClearAllItems: "Confirm Clear All Items",
      ConfirmRemoveAll: "Are you sure you want to remove all {{count}} items? This action cannot be undone.",
      Cancel: "Cancel",
      Price: "Price",
      Item: "Item",
      Quantity: "Quantity",
      Preparation: "Preparation",
      Pay: "Pay",
      Total: "Total",
      Void: "Void",
      NoItemsFound: "No items found for this order.",
      TotalOrderAmount: "Total Order Amount",
      EGP: "EGP",
      AmountToPay: "Amount To Pay :",
      Checkout: "Checkout",
      SaveasPending: "Save as Pending",
      ApplyOfferUsePoints: "Apply Offer / Use Points",
      EnterLoyaltyOrRewardCode: "Enter the customer's loyalty code or the reward item code.",
      CheckCode: "Check Code",
      ConfirmRewardPurchase: "Confirm Reward Purchase",
      UserID: "User ID",
      ConfirmAddOffer: "Confirm adding {{product}} to the order for ({{points}} Points)?",
      ApproveandAddItem: "Approve and Add Item",
      EnterDealCode: "Enter the customer's deal code to check validity.",
      ApplyDealCode: "Apply Deal Code",
      CheckDeal: "CheckDeal",
      ConfirmDealAcceptance: "Confirm Deal Acceptance",
      CustomerUserId: "Customer: User ID **{{user_id}}**",
      ConfirmAddDeal: "Confirm adding {{deal_title}} for ({{deal_price}} EGP) to the order?",
      PendingOrders: "Pending Orders",
      SubTotal: "Sub Total:",
      Tax: "Tax:",
      OtherCharge: "OtherCharge:",
      "Cart Has Items": "Cart Has Items",
      "You have": "You have",
      "item(s) in your cart. What would you like to do?": "item(s) in your cart. What would you like to do?",
      "Clear & Switch": "Clear & Switch",
      "Keep for Later": "Keep for Later",
      //item
      QuantityIncreased: "The quantity of \"{{name}}\" has been increased to {{quantity}}.",
      UpdateFailed: "Failed to update \"{{name}}\".",
      ProductAdded: "\"{{name}}\" added successfully!",
      SubmitFailed: "Failed to submit \"{{name}}\".",
      ErrorLoadingData: "Error loading data. Please try again later. Check your authentication token."
      , SelectBranchToViewItems: "Please select a branch to view items."
      , TakeAway: "Take Away",
      SelectCategory: "Select Category",
      ByPiece: "By Piece",
      ByWeight: "By Weight",
      SearchByProductName: "Search by product name...",
      Noproductsfoundfor: "No products found for",
      SelectedCategory: "Selected Category",
      ShowMoreProducts: "Show More Products",
      // Cashier
      SelectionCashier: "Selection Cashier",
      Nocashiersavailable: "No cashiers available",
      ShowMore: "Show More",
      //login
      NotificationsDisabled: "Notifications disabled; some features may be limited",
      FailedToEnableNotifications: "Failed to enable notifications",
      PleaseFillInAllFields: "Please fill in all fields",
      LoggedInSuccessfully: "Logged in successfully",
      AnErrorOccurred: "An error occurred",
      LoginTitle: "Log in to Food2go",
      WelcomeBack: "Welcome back",
      Username: "Username",
      Password: "Password",
      Login: "Login",
      //shift
      ShiftOpenedSuccessfully: "Shift opened successfully! Redirecting to home...",
      FailedToOpenShift: "Failed to open shift.",
      ShiftClosedSuccessfully: "Shift closed successfully!",
      FailedToCloseShift: "Failed to close shift",
      ShiftStatus: 'Shift Status',
      CurrentlyOnShift: "You're currently on shift",
      UpForShift: "You're up for your shift",
      TakeYourShift: "Take your shift",
      BackToWork: "Back to Work"
      , OpenNewShift: "Open New Shift",
      Redirectingtohomein: "Redirecting to home in",
      Description: "Description",
      Address: "Address",
      //productcard
      Adding: "Adding...",
      AddToOrder: "Add to Order",
      //Delivery
      AddUser: "Add User",
      Nousersfoundmatchingyoursearch: "No users found matching your search",
      Noaddressavailable: "No address available",
      AddAddress: "Add Address",
      ConfirmDelivery: "Confirm Delivery",
      Pleasesearchbynameorphonetofindusers: "Please search by name or phone to find users",
      Edituser: "Edit user",
      FirstName: "First Name",
      LastName: "Last Name",
      Phone: "Phone",
      AnotherPhone: "Another Phone",
      Saving: "Saving",
      save: "Save",
      SearchuserNameorPhone: "Search user Name or Phone",
      Nodescriptionavailable: "No description available",
      Min: "Min",
      Max: "Max",
      ExtrasOptional: "Extras (Optional)",
      Addons: "Addons",
      Variations: "Variations",
      Free: "Free",
      AddonsOptional: "Addons (Optional)",
      Weight: "Weight",
      AddtoCart: "Add to Cart",
      ExcludeOptional: "Exclude (Optional)",
      SpecialInstructionsOptional: "Special Instructions (Optional)",
      characters: "characters",
      AddSpecialInstructions: "Add any special instructions for this item...",
      Available: "Available",
      Preorder: "Pre-order",
      Withorder: "With order",
      Reserved: "Reserved",
      CannotTransferToSameTable: "Cannot transfer to the same table. Please select a different table."
      , OrderTransferredSuccessfully: "Order transferred successfully!",
      FailedtotransfertablePleasetryagain: "Failed to transfer table. Please try again.",
      MergedTable: "Merged Table",
      Cap: "Cap",
      Source: "Source",
      Customer: "Customer",
      Transferringorder: "Transferring order...",
      Failedtoloadtables: "Failed to load tables",
      Pleasecheckyournetworkconnectionortryagainlater: "Please check your network connection or try again later",
      SelectNewTableForTransfer: "Select New Table for Transfer",
      DineInTables: "Dine-in Tables",
      NoTablesFound: "No tables found for this location.",
      SelectAnotherLocationOrAddTables: "Please select another location or add tables.",
      NoCafeLocationsFound: "No cafe locations found for this branch.",
      EnsureLocationsConfigured: "Please ensure locations are configured in the system.",
      SelectNewTableToTransferOrder: "Please select a new table to transfer the order.",
      SelectTableToTransferFrom: "Select a table to transfer the order from Table {{sourceTableId}}.",
      SelectTableToStartOrder: "Select a table to start an order. Merged tables are highlighted in purple."
      //DeliveryInfo
      , DeliveryInformation: "Delivery Information",
      Failedtoloaduserinformation: "Failed to load user information ❌",
      CustomerName: "Customer Name",
      PhoneNumber: "Phone Number",
      DeliveryAddress: "Delivery Address",
      Nouserinformationavailable: "No user information available ⚠️",
      Pleaseselectacustomerfromthedeliverypage: "Please select a customer from the delivery page"
      ,
      //constants
      Pending: "Pending",
      Waiting: "Waiting",
      Preparing: "Preparing",
      PickUp: "Pick Up",
      Done: "Done",
      Notes: "Notes",
      //ordersview

      ApproveandAddDeal: "Approve and Add Deal",
      Statusupdatedsuccessfully: "Status updated successfully",
      Errorupdatingstatus: "Error updating status",
      Orders: "Orders:",
      SearchOrderNumber: "Search Order Number",
      NoOrdersFound: "No orders found",
      Updating: "Updating...",
      DineIn: "Dine In",
      Items: "Items",
      EnterOfferCode: "Enter Offer Code",
      ApproveandAddOffer: "Approve and Add Offer",
      delivery: "Delivery",
      //DeliveryAdd
      Pleaseselectacity: "Please select a city",
      Pleaseselectazone: "Please select a zone",
      Pleaseprovideavalidaddress: "Please provide a valid address",
      AddressIDnotfound: "Address ID not found",
      Addressupdated: "Address updated",
      Addressadded: "Address added",
      Useradded: "User added",
      successfully: "successfully",
      Operationfailed: "Operation failed",
      FailedtosubmitPleasetryagain: "Failed to submit. Please try again.",
      Unknownerror: "Unknown error",
      AddAnotherAddressforUser
        : "Add Another Address for User",
      AddNewDeliveryUser: "Add New Delivery User",
      EditDeliveryAddress: "Edit Delivery Address",
      AddressSelectionMode: "Address Selection Mode",
      Automaticfrommaplocation: "Automatic from map location",
      Manualaddressentry: "Manual address entry",
      Manual: "Manual",
      fromMap: "(from Map)",
      Auto: "Auto",
      ManualEntry: "(Manual Entry)",
      Clickotaddress: "Click on map to select address",
      Enteryouraddressmanually: "Enter your address manually",
      Street: "Street",
      Building: "Building",
      Floor: "Floor",
      Apartment: "Apartment",
      Landmark: "Landmark",
      AdditionalNotes: "Additional Notes",
      SelectType: "Select Type",
      Home: "Home",
      Work: "Work",
      Other: "Other",
      AdditionalData: "Additional Data",
      Zone: "Zone",
      City: "City",
      SelectZone: "Select Zone",
      SelectCity: "Select City",
      Type: "Type",
      UpdateAddress: "Update Address",
      Errorloadingpendingorders: "Error loading pending orders",
      Errorloadingorderdetails: "Error loading order details",
      Retry: "Retry",
      Nopendingordersfound: "No pending orders found",
      Loadingorderdetails: "Loading order details",
      Imagesizemustbelessthan2MB: "Image size must be less than 2MB",
      Usernameisrequired: "Usernameisrequired",
      Passwordmustbeatleast3characters: "Password must be at least 3 characters",
      Profileupdatedsuccessfully: "Profile updated successfully",
      Errorupdatingprofile: "Error updating profile",
      EditProfile: "Edit Profile",
      Inactive: "Inactive",
      Active: "Active",
      ProfileImage: "Profile Image",
      Failedtoloaddueusers: "Failed to load due users",
      Nouserswithdueamounts: "No users with due amounts",
      DueUsers: "Due Users",
      PayNow: "Pay Now",
      ReceptionHall: "Reception Hall",
      MainHall: "Main Hall",
      OnlineOrders: "Online Orders",
      OrderNumber: "Order #",
      Branch: "Branch",
      Amount: "Amount",
      Status: "Status",
      DateTime: "Date/Time",
      TotalOrders: "Total Orders",
      SearchByOrderNumber: "Search by order number, name, or phone...",
      AllStatuses: "All Statuses",
      //
      EnterPassword: "Enter Password",
      NoOrdersFoundForThisDate: "No orders found for this date.",
      Notifications: "Notifications",
      NoNotifications: "NoNotifications",
      ClearAll: 'Clear All',
      AddExpense: "Add Expense",
      Expense: "Expense",
      Category: "Category",
      EnterExpense: "EnterExpense",
      Enteramount: "Enter amount",
      FinancialAccount: "Financial Account",
      SelectAccount: "Select Account",
      Note: "Note",
      add: "Add",
      //
      ProcessPayment: "Process Payment :",
      OriginalAmount: "Original Amount :",
      Discount: "Discount :",
      Remaining: "Remaining :.",
      Change: "Change",
      //
      TotalAmount: "Total Amount",
      OrderNotes: "Order Notes (Optional)",
      OrderNotesPlaceholder: "Add any special instructions or notes for this order...",
      DiscountbyCompany: "Discount by Company",
      EnterDiscountCode: "Enter discount code",
      Checking: "Checking...",
      Apply: "Apply",
      DiscountAppliedSuccess: "Discount of {{appliedDiscount}}% applied successfully!",
      Remove: "Remove",
      AddAccountSplit: "+ Add account split",
      AmountPaidByCustomer: "Amount Paid by Customer",
      EnterAmountPaid: "Enter amount paid",
      ChangeDue: "Change Due: {{value}} EGP",
      MarkAsDueOrder: "Mark as Due Order (Customer will pay later)",
      Processing: "Processing...",
      DueOrderReady: "Due Order Ready",
      SelectCustomer: "Select Customer",
      ConfirmAndPay: "Confirm & Pay",
      ProductAddedToCart: "Product added to cart",
      Favorite: "Favorite",
      NoCategoriesFound: "No categories found.",

      ExpenseAdded: "Expense Added",
      Failedtoaddexpense: "Failed to add expense",
      Pleasefillrequiredfields: "Please fill required fields",
      Pleaseselectfinancialaccount: "Please select financial account",
      Accessgrantedsuccessfully: "Access granted successfully",
      Incorrectpassword: "Incorrect password",
      totheserver: "An error occurred while connecting to the serve",
      Pleaseenteryourpassword: "Please enter your password",
      QZTrayNotRunning: "QZ Tray service not running. Automatic printing is disabled.",
      PleaseEnterDiscountCode: "Please enter a discount code.",
      DiscountApplied: "Discount of {{discount}}% applied!",
      InvalidOrOffDiscountCode: "Invalid or inactive discount code.",
      FailedToValidateDiscountCode: "Failed to validate discount code.",
      AmountCannotBeNegative: "Amount cannot be negative.",
      AmountExceedsLimit: "Amount cannot exceed: {{amount}} EGP.",
      NoFinancialAccounts: "No financial accounts available.",
      DueOrderCreated: "Due order created successfully!",
      OrderPlaced: "Order placed successfully!"
      , FailedToProcessOrder: "Failed to process order.",
      SubmissionFailed: "Submission failed",
      OrderExceedsDebitLimit: "Order amount ({{amount}} EGP) exceeds customer's debit limit.",
      TotalMustEqual: "Total must equal {{amount}} EGP.",
      OrderIdMissing: "Order ID is missing.",
      SelectDeliveryPerson: "Please select a delivery person.",
      DeliveryPersonAssigned: "Delivery person assigned successfully!",
      FailedToAssignDeliveryPerson: "Failed to assign delivery person.",
      PhoneNumber2Optional: "Phone Number 2 (Optional)",

      CurrentHall: "Current Hall",
      Table: "Table",
      ProductAddedToTable: "Product Added To Table",
      VoidItemManagerAuthentication: "Void Item Manager Authentication",
      ManagerID: "Manager ID",
      EnterManagerID: "Enter Manager ID",
      Voiding: "Voiding",
      ConfirmVoid: "Confirm Void",
      ReadyForPayment: "Ready for Payment",
      items: "items",
      SelectAll: "Select All",
      DeselectAll: "Deselect All",
      SelectItemsForPayment: "Select items you want to process payment for ({{count}} selected)",
      ServiceFee: "Service Fee",
      EndShiftReport: "End Shift Report",
      Employee: "Employee",
      ShiftDuration: "Shift Duration",
      FinancialSummary: "Financial Summary",
      TotalCashInShift: "Total Cash in Shift",
      OrdersSummaryByType: "Orders Summary by Type",
      PaymentMethodsBreakdown: "Payment Methods Breakdown",
      TotalAllOrders: "Total All Orders",
      Expenses: "Expenses",
      TotalExpenses: "Total Expenses",
      NetCashInDrawer: "Net Cash in Drawer",
      TotalSales: "Total Sales",
      NetCash: "Net Cash",
      ConfirmCloseShift: "Confirm Close Shift",
      Unknown: "Unknown",
      ConfirmShiftClosure: "Confirm Shift Closure",
      EnterPasswordToCloseShift: "Enter your password to close the shift",
      Confirm: "Confirm",
      price: "Price: {{value}}",
      category: "Category: {{name}}",
      note: "Note: {{note}}",

      Order: "Order",
      OrderTime: "Order Time"
      , OrderDate: "Order Date",
      Schedule: "Schedule",
      PaymentMethod: "Payment Method",
      PaymentStatus: "Payment Status",
      TransactionID: "Transaction ID",
      OrderType: "Order Type",
      OrderNote: "Order Note",

      OrderRecipt: "Order Recipt",
      Receipt: "Receipt",
      NoRecipt: "No Recipt",
      OrderItems: "Order Items",
      Products: "Products",
      variations: "variations",
      Extra: "Extras",
      Excludes: "Excludes",

      AddonsPrice: "Addons Price",
      ItemsPrice: "Items Price",
      Subtotal: "Subtotal",
      ExtraDiscount: "Extra Discount",
      CouponDiscount: "Coupon Discount",
      DeliveryFee: "Delivery Fee",
      TaxVAT: "Tax/VAT",

      CustomerInformation: "Customer Information",
      Name: "Name",
      Contact: "Contact",
      Email: "Email",
      BuildNum: "Building Number",
      House: "House",
      Room: "Room",
      Road: "Road",
      LocationMap: "Location Map",
      TransferToAnotherBranch: "Transfer To Another Branch",
      TransferToAnotherLocation: "Transfer To Another Location",
      TransferOrder: "Transfer Order",
      TransferOrderToAnotherBranch: "Transfer Order To Another Branch",
      TransferOrderToAnotherLocation: "Transfer Order To Another Location",
      EnterCancelReason: "Enter Cancel Reason",


      ChangeOrderStatus: "Change Order Status",
      CancelOrder: "Cancel Order",
      CancelOrderReason: "Cancel Order Reason",
      CancelOrderReasonPlaceholder: "Enter the reason for canceling the order",
      Pleaseenteracancellationreason: "Please enter a cancellation reason",
      ConfirmCancellation: "Confirm Cancellation",
      AdminCancellationReason: "Admin Cancellation Reason",
      CustomerCancellationReason: "Customer Cancellation Reason",

      Entercancellationreason: "Enter cancellation reason...",
      Areyousureyouwanttorefundthisorder: "Are you sure you want to refund this order?",
      ConfirmRefund: "Confirm Refund",
      NoCancel: "No, Cancel",
      RefundOrder: "Refund Order",
      YesRefund: "Yes, Refund",
      RefundReason: "Refund Reason",
      AssignDeliveryMan: "Assign Delivery Man",
      NotFoundDelivery: "Delivery man not found",
      Pleaseselectadeliveryperson: "Please select a delivery person",
      AssignDelivery: "Assign Delivery",
      FoodPreparationTime: "Food Preparation Time",
      Preparingtimenotavailable: "Preparing time not available",

      DeliveryMan: "Delivery Man",
      OrderinUsebyAnotherPerson: "Order in Use by Another Person",
      Someoneelseis: "Someone else is currently working on this order. Please wait until they finish before proceeding to avoid conflicts or duplication."
      ,
      Ok: "Ok",
      TransferOrderToBranch: "Transfer Order To Branch",
      SelectBranchToTransfer: "Select Branch To Transfer",
      CurrentBranch: "Current Branch",
      NoBranchesAvailable: "No branches available",
      Accept: "Accept",
      OutforDelivery: "Out for Delivery",
      Delivered: "Delivered",
      FailedtoDeliver: "Failed to Deliver",
      Returned: "Returned",
      Canceled: "Canceled",
      Refund: "Refund",
      ConfirmOrder: "Confirm Order",
      ConfirmOrderWithDueModule: "Confirm Order with Due Module ({{amount}} EGP)",
      PlatformWillPayRemaining: "Platform Will Pay Remaining",
      ChooseStatus: "Choose Status",
      PleaseEnterModuleOrderNumber: "Please enter module order number",
      EnterModuleOrderNumber: "Enter module order number",
      ModuleOrderNumber: "Module Order Number",
      Save: "Save",
      UnknownProduct: "Unknown Product",
      UnknownAddon: "Unknown Addon",
      CashInHandAmount: "How much cash is currently in hand?",
      ConfirmAndSend: "Confirm and Send",
      Loading: "Loading...",
      SelectCustomerForDueOrder: "Select Customer for Due Order",
      OrderTotal: "Order Total",
      DebitLimit: "Debit Limit",
      LimitExceeded: "Limit Exceeded",
      NoCustomersFoundRefineSearch: "No customers found. Please refine your search.",
      Searching: "Searching...",
      OrderExceedsDebitLimitWithValues: "Order amount ({{amount}} EGP) exceeds customer's debit limit ({{limit}} EGP).",
      OrdersModeActivated: "Orders mode activated",
      FailedToLoadOrders: "Failed to load orders",
      PleaseSelectDateRange: "Please select a date range",
      ThankYouForVisit: "Thank You For Your Visit",
      DeliveryFees: "Delivery Fees",
      CustomerInfo: "Customer Info",
      NoOrdersFoundClickSearch: "No orders found - click Search after selecting dates",
      NoOrdersMatchFilters: "No orders match the current filters",
      FailedToRefreshOrders: "Failed to refresh orders",
      More: "more",
      New: "New",
      User: "user",
      UserAccount: "User Account",
      NormalPrices: "Normal Prices",
      RegularMenuPrices: "Regular Menu Prices",
      NewOrderReceived: "New Order Received!",
      Categories: "Categories",
      Offers: "Offers",
      OrderNotes: "Order Notes",
      Points: "Points",
      TransferDineIn: "Transfer To Dine In",
      DeliveryFee: "Delivery Fee",
      AmountbyCustomer: "Amount by Customer",
      PayNow: "Pay Now",
      MergeTables: "Merge Tables",
      SelectCaptain: "Select Captain",
      SelectDateRange: "Select Date Range",
      OrderType: "Order Type",
      StartDate: "Start Date",
      EndDate: "End Date",
      Search: "Search",
      Reset: "Reset",
      ShowData: "Show Data",
      AllOrders: "All Orders",
      Pending: "Pending",
      confirmed: "Confirmed",
      Accept: "Accept",
      OutforDelivery: "Out For Delivery",
      delivered: "Delivered",
      returned: "Returned",
      canceled: "Canceled",
      refund: "Refund",
      FailedToDeliver: "Failed To Deliver",
      DeliveryMan: "Delivery Man",
      Due: "Due",
      Tables: "Tables",
      Reports: "Reports",
      ViewInvoice: "View Invoice",
      ExtrasPrice: "Extras Price",
      Payment: "Payment",
      DeliveryOrder: "Delivery Order",
      DineInOrder: "Dine In Order",
      NewOrders: "New Orders",
      Noneworders: "No New Orders",
      Print: "Print",
      Selected: "Selected",
      Previous: "Previous",
      Next: "Next",
      Page: "Page",
      of: "of",
      From: "From",
      To: "To",
      OrderID: "Order ID",
      OrderNumber: "Order Number",
      Customer: "Customer",
      Phone: "Phone",
      Address: "Address",
      Amount: "Amount",
      Date: "Date",
      SelectAll: "Select All",
      ClearSelection: "Clear Selection",
      AssignSelectedOrders: "Assign Selected Orders",
      Page: "Page",
      of: "of",
      Next: "Next",
      Previous: "Previous",
      ChooseDeliveryMan: "Choose Delivery Man",
      Current: "Current",
      History: "History",
      Returned: "Returned",
      AllOrders: "All Orders",
      Current: "Current",
      Returned: "Returned",
      History: "History",
      OrderTypesSummary: "Order Types Summary",
      NumberOfOrders: "Number Of Orders",
      TotalAmount: "Total Amount",
      TotalCashInShift: "Total Cash In Shift",
      Expenses: "Expenses",
      Returns: "Returns",
      ActualTotalInDrawer: "Actual total in drawer",
      NetCashInDrawer: "Net cash in drawer",
      CashShortage: "Cash shortage / Gap",
      TotalSales: "Total sales",
      AllPaymentMethods: "All payment methods",
      PrintedAt: "Printed at",
      CashierName: "Cashier Name",
      POSMachine: "POS Machine",
      ShiftOpenedAt: "Shift Opened At",
      ShiftClosedAt: "Shift Closed At",







    },
  },
  ar: {
    translation: {
      Ok: "موافق",
      Selected: "تم الاختيار",
      OrderTypesSummary: "ملخص أنواع الطلبات",
      From: "من",
      To: "إلى",
      CashierName: "اسم الكاشير",
      PrintedAt: "تم الطباعة في",
      POSMachine: "جهاز نقاط البيع",
      ShiftOpenedAt: "تم فتح الشفت في",
      ShiftClosedAt: "تم إغلاق الشفت في",
      NumberOfOrders: "عدد الطلبات",
      TotalAmount: "المبلغ الإجمالي",
      ClearSelection: "مسح التحديد",
      TotalCashInShift: "إجمالي النقد في الشفت",
      Expenses: "المصروفات",
      Returns: "المرتجعات",
      ActualTotalInDrawer: "إجمالي النقد الفعلي في الدرج",
      NetCashInDrawer: "صافي النقد في الدرج",
      CashShortage: "نقص النقد / الفجوة",
      TotalSales: "إجمالي المبيعات",
      AllPaymentMethods: "جميع طرق الدفع",
      OrderID: "رقم الطلب",
      of: "من",
      Page: "الصفحة",
      Next: "التالي",
      Previous: "السابق",
      Current: "الحالي",
      History: "التاريخ",
      Returned: "المُرجع",
      AllOrders: "جميع الطلبات",
      Current: "الحالي",
      Returned: "المُرجع",
      History: "التاريخ",
      ChooseDeliveryMan: "اختر الكابتن",
      OrderNumber: "رقم الطلب",
      Customer: "العميل",
      Phone: "الهاتف",
      Address: "العنوان",
      Amount: "المبلغ",
      Date: "التاريخ",
      SelectAll: "تحديد الكل",
      ClearSelection: "مسح التحديد",
      AssignSelectedOrders: "تعيين الطلبات المحددة",
      To: "إلى",
      Previous: "السابق",
      Next: "التالي",
      Page: "الصفحة",
      of: "من",
      Accept: "قبول",
      Print: "طباعة",
      NewOrders: "طلبات جديدة",
      Noneworders: "لا يوجد طلبات جديدة",
      DeliveryOrder: "طلب توصيل",
      DineInOrder: "طلب داين إن",
      Payment: "الدفع",
      ExtrasPrice: "سعر الإضافات",
      ViewInvoice: "عرض الفاتورة",
      Due: "المديونين",
      Reports: "التقارير",
      Tables: "الطاولات",
      StartDate: "تاريخ البدء",
      EndDate: "تاريخ الانتهاء",
      Search: "بحث",
      Reset: "إعادة تعيين",
      ShowData: "عرض البيانات",
      AllOrders: "جميع الطلبات",
      Pending: "قيد الانتظار",
      confirmed: "تم التأكيد",
      Accept: "تم القبول",
      outforDelivery: "تم التوصيل",
      delivered: "تم التوصيل",
      returned: "تم الإرجاع",
      canceled: "تم الإلغاء",
      Refund: "تم الاسترجاع",
      FailedToDeliver: "فشل في التوصيل",
      OrderType: "نوع الطلب",
      DeliveryMan: "الكابتن",
      SelectCaptain: "اختر الكابتن",
      SelectDateRange: "اختر نطاق التاريخ",
      Categories: "التصنيفات",
      MergeTables: "دمج الطاولات",
      PayNow: "ادفع الآن",
      AmountbyCustomer: "المبلغ من العميل",
      DeliveryFee: "رسوم التوصيل",
      Points: "النقاط",
      TransferDineIn: "نقل إلى داين إن",
      OrderNotes: "ملاحظات الطلب",
      Offers: "العروض",
      OutforDelivery: "خرج للتوصيل",
      Delivered: "تم التوصيل",
      FailedtoDeliver: "فشل في التوصيل",
      Returned: "تم الإرجاع",
      Canceled: "ملغى",
      Refund: "استرجاع",
      TransferOrderToBranch: "نقل الطلب إلى الفرع",
      SelectBranchToTransfer: "اختر الفرع للنقل",
      CurrentBranch: "الفرع الحالي",
      NoBranchesAvailable: "لا توجد فروع متاحة"
      , TransferToAnotherBranch: "النقل إلى فرع آخر"
      ,
      Areyousureyouwanttorefundthisorder: "هل أنت متأكد أنك تريد استرجاع هذا الطلب؟",
      ConfirmRefund: "تأكيد الاسترجاع",
      NoCancel: "لا، إلغاء",
      RefundOrder: "استرجاع الطلب",
      YesRefund: "نعم، استرجاع",
      RefundReason: "سبب الاسترجاع",
      AssignDeliveryMan: "تعيين مندوب التوصيل",
      NotFoundDelivery: "مندوب التوصيل غير موجود",
      Pleaseselectadeliveryperson: "يرجى اختيار مندوب التوصيل",
      AssignDelivery: "تعيين التوصيل",
      FoodPreparationTime: "وقت تجهيز الطعام",
      Preparingtimenotavailable: "وقت التجهيز غير متاح",
      Entercancellationreason: "ادخل سبب الاغلاق",
      ChangeOrderStatus: "تغيير حالة الطلب",
      CancelOrder: "إلغاء الطلب",
      CancelOrderReason: "سبب إلغاء الطلب",
      CancelOrderReasonPlaceholder: "أدخل سبب إلغاء الطلب",
      Pleaseenteracancellationreason: "يرجى إدخال سبب الإلغاء",
      ConfirmCancellation: "تأكيد الإلغاء",
      AdminCancellationReason: "سبب إلغاء المسؤول",
      CustomerCancellationReason: "سبب إلغاء العميل",
      BuildNum: "رقم المبنى",
      House: "المنزل",
      Room: "الغرفة",
      Road: "الشارع",
      LocationMap: "خريطة الموقع",
      CustomerInformation: "معلومات العميل",
      Name: "الاسم",
      Contact: "جهة الاتصال",
      Email: "البريد الإلكتروني",
      TaxVAT: "الضريبة / ضريبة القيمة المضافة"
      , AddonsPrice: "سعر الإضافات",
      ItemsPrice: "سعر العناصر",
      Subtotal: "المجموع الفرعي",
      ExtraDiscount: "خصم إضافي",
      CouponDiscount: "خصم الكوبون",
      DeliveryFee: "رسوم التوصيل",
      OrderRecipt: "إيصال الطلب",
      Receipt: "الإيصال",
      NoRecipt: "لا يوجد إيصال",
      OrderItems: "عناصر الطلب",
      Products: "المنتجات",
      variations: "الخيارات",
      Extra: "الإضافات",
      Excludes: "الاستثناءات",
      Order: "الطلب",
      OrderTime: "وقت الطلب",
      OrderDate: "تاريخ الطلب",
      Schedule: "الجدول",
      PaymentMethod: "طريقة الدفع",
      PaymentStatus: "حالة الدفع",
      TransactionID: "رقم العملية",
      OrderType: "نوع الطلب",
      OrderNote: "ملاحظة الطلب",

      price: "السعر: {{value}}",
      category: "التصنيف: {{name}}",
      note: "ملاحظة: {{note}}",
      ServiceFee: "رسوم الخدمة"
      , TotalAmount: "مجموع الحساب",
      ManagerID: "رقم هوية المدير",
      Voiding: "جاري الإلغاء",
      ConfirmVoid: "تأكيد الإلغاء"
      , ReadyForPayment: "جاهز للدفع",
      items: "عناصر",
      SelectAll: "اختر الكل",
      DeselectAll: "إلغاء تحديد الكل",
      SelectItemsForPayment: "اختر العناصر التي تريد معالجتها للدفع ({{count}} محددة)",
      EnterManagerID: "أدخل رقم هوية المدير",
      CurrentHall: "القاعة الحالية",
      Table: "الطاولة"
      , ProductAddedToTable: "تم إضافة المنتج للطاولة",
      VoidItemManagerAuthentication: "إلغاء العنصر - مصادقة المدير",
      ExpenseAdded: "تمت إضافة المصروف",
      Failedtoaddexpense: "فشل في إضافة المصروف",
      Pleasefillrequiredfields: "يرجى تعبئة الحقول المطلوبة",
      Pleaseselectfinancialaccount: "يرجى اختيار الحساب المالي",
      Accessgrantedsuccessfully: "تم منح الوصول بنجاح",
      Incorrectpassword: "كلمة المرور غير صحيحة",
      totheserver: "حدث خطأ أثناء الاتصال بالخادم",
      Pleaseenteryourpassword: "يرجى إدخال كلمة المرور الخاصة بك",
      QZTrayNotRunning: "خدمة QZ Tray غير مفعلة. الطباعة التلقائية غير ممكنة.",
      PleaseEnterDiscountCode: "يرجى إدخال رمز الخصم",
      DiscountApplied: "تم تطبيق خصم بنسبة {{discount}}%",
      InvalidOrOffDiscountCode: "رمز الخصم غير صالح أو غير مفعل",
      FailedToValidateDiscountCode: "فشل في التحقق من رمز الخصم",
      AmountCannotBeNegative: "المبلغ لا يمكن أن يكون سالبًا",
      AmountExceedsLimit: "المبلغ لا يمكن أن يتجاوز: {{amount}} جنيه",
      NoFinancialAccounts: "لا توجد حسابات مالية متاحة",
      DueOrderCreated: "تم إنشاء طلب مؤجل بنجاح",
      OrderPlaced: "تم تقديم الطلب بنجاح",
      FailedToProcessOrder: "فشل في معالجة الطلب",
      SubmissionFailed: "فشل الإرسال",
      OrderExceedsDebitLimit: "مبلغ الطلب ({{amount}} جنيه) يتجاوز حد الدين للعميل",
      TotalMustEqual: "يجب أن يساوي المجموع {{amount}} جنيه",
      OrderIdMissing: "رقم الطلب مفقود",
      SelectDeliveryPerson: "يرجى اختيار موظف التوصيل",
      DeliveryPersonAssigned: "تم تعيين موظف التوصيل بنجاح",
      FailedToAssignDeliveryPerson: "فشل في تعيين موظف التوصيل",
      PhoneNumber2Optional: "رقم الهاتف 2 (اختياري)"
      ,
      NoCategoriesFound: "لا توجد فئات.",

      Favorite: "المفضلة ",
      Remove: "إزالة",
      DiscountbyCompany: "خصم من الشركة",
      ProductAddedToCart: "تم إضافة المنتج إلى السلة",

      MarkAsDueOrder: "تحديد كطلب آجل (العميل سيدفع لاحقًا)",
      Processing: "جاري المعالجة...",
      DueOrderReady: "الطلب الآجل جاهز",
      SelectCustomer: "اختر العميل",
      ConfirmAndPay: "تأكيد و دفع",

      AddAccountSplit: "+ إضافة تقسيم حساب",
      AmountPaidByCustomer: "المبلغ المدفوع من العميل",
      EnterAmountPaid: "أدخل المبلغ المدفوع",
      ChangeDue: "المبلغ المسترجع: {{value}} جنيه",
      OrderNotesPlaceholder: "أضف أي ملاحظات أو تعليمات خاصة لهذا الطلب..."
      , EnterDiscountCode: "أدخل رمز الخصم"
      , Checking: "جارٍ التحقق...",
      Apply: "تطبيق",
      DiscountAppliedSuccess: "تم تطبيق خصم بنسبة {{appliedDiscount}}٪ بنجاح!",

      OrderNotes: "ملاحظات الطلب (اختياري)",
      add: "أضافة ",
      ProcessPayment: "إتمام الدفع :",
      OriginalAmount: "المبلغ الأصلي :",
      Discount: "الخصم :",
      Remaining: "المتبقي :",
      Change: "الباقي"
      ,

      EnterPassword: "أدخل كلمة المرور",
      NoOrdersFoundForThisDate: "لا توجد طلبات في هذا التاريخ",
      Notifications: "الإشعارات",
      NoNotifications: "لا توجد إشعارات",
      ClearAll: "مسح الكل",
      AddExpense: "إضافة مصروف",
      Expense: "مصروف",
      Category: "الفئة",
      EnterExpense: "أدخل المصروف",
      Enteramount: "أدخل المبلغ",
      FinancialAccount: "الحساب المالي",
      SelectAccount: "اختر الحساب",
      Note: "ملحوظة",
      SearchByOrderNumber: "ابحث برقم الطلب أو الاسم أو الهاتف...",
      AllStatuses: "جميع الحالات",
      TotalOrders: "مجموع الأوردات",

      OnlineOrders: "الطلبات أونلاين",
      title: "فود تو جو",
      profile: "الملف الشخصي",
      dueUsers: "العملاء المتأخرين",
      closeshift: "إغلاق الوردية",
      logout: "تسجيل الخروج",
      exit: "خروج",
      code: "رمز",
      OrderNumber: "رقم الطلب",
      Customer: "العميل",
      Branch: "الفرع",
      Amount: "المبلغ",
      Status: "الحالة",
      DateTime: "التاريخ / الوقت",
      Type: "النوع",
      shift: "الوردية",
      ProfileImage: "صورة الملف الشخصي",
      Failedtoloaddueusers: "فشل في تحميل العملاء المتأخرين",
      Nouserswithdueamounts: "لا يوجد عملاء بمبالغ متأخرة",
      DueUsers: "العملاء المتأخرين",
      PayNow: "ادفع الآن",
      MainHall: "الصالة الرئيسية",
      ReceptionHall: "صالة الاستقبال",
      //home
      Failedtofetchdiscountdata: "فشل في جلب بيانات الخصم.",
      IncompletetransferdataCannotcompletetransfer: "بيانات النقل غير مكتملة. لا يمكن إكمال النقل.",
      OrderTransferred: "تم نقل الطلب بنجاح من الطاولة {{sourceTableId}} إلى الطاولة {{newTableId}}.",
      FailedtocompletetransferPleasetryagain: "فشل في إكمال النقل. يرجى المحاولة مرة أخرى.",
      take_away: "تيك أواي ",
      Delivery: "توصيل",
      Dinein: "صالة ",
      //login
      NotificationsDisabled: "تم تعطيل الإشعارات؛ قد تكون بعض الميزات محدودة",
      FailedToEnableNotifications: "فشل في تمكين الإشعارات",
      PleaseFillInAllFields: "يرجى ملء جميع الحقول",
      LoggedInSuccessfully: "تم تسجيل الدخول بنجاح",
      AnErrorOccurred: "حدث خطأ",
      LoginTitle: "تسجيل الدخول إلى فود تو جو",
      WelcomeBack: "مرحبًا بعودتك",
      Username: "اسم المستخدم",
      Password: "كلمة المرور",
      Login: "تسجيل الدخول",

      //navbar
      YouMustCloseShiftBeforeLogout: "يجب إغلاق الوردية قبل تسجيل الخروج.",
      LoggedOutSuccessfully: "تم تسجيل الخروج بنجاح",
      ErrorWhileLoggingOut: "حدث خطأ أثناء تسجيل الخروج",
      //card
      Allitemsclearedfromtheorder: "تم مسح جميع العناصر من الطلب.",
      Noitemstosaveaspending: "لا توجد عناصر لحفظها كمعلقة.",
      Nospecialinstructions: "لا تعليمات خاصة",
      Ordersavedaspending: "تم حفظ الطلب كمعلق!",
      Failedtosaveaspending: "فشل في الحفظ كمعلق.",
      Pleaseenteranoffercode: "يرجى إدخال رمز العرض.",
      OffervalidatedsuccessfullyPleaseconfirm: "تم التحقق من العرض بنجاح! يرجى التأكيد.",
      Offerdetailsareincompleteintheresponse: "تفاصيل العرض غير مكتملة في الاستجابة",
      FailedtovalidateofferPleasetryagain: "فشل في التحقق من العرض. يرجى المحاولة مرة أخرى.",
      RewardAdded: "تم إضافة العنصر \"{{product}}\" بنجاح إلى الطلب",
      Failedtoapproveoffer: "فشل في الموافقة على العرض.",
      Pleaseenteradealcode: "يرجى إدخال رمز الصفقة.",
      DealvalidatedsuccessfullyPleaseconfirm: "تم التحقق من الصفقة بنجاح! يرجى التأكيد.",
      Unexpectedresponsefromserverorinvaliddeal: "استجابة غير متوقعة من الخادم أو صفقة غير صالحة",
      FailedtoapplydealPleasetryagain: "فشل في تطبيق الصفقة. يرجى المحاولة مرة أخرى.",
      DealAdded: "تم إضافة العرض \"{{deal_title}}\" بنجاح إلى الطلب!",
      CannottransferorderTableIDorCartIDsaremissing: "لا يمكن نقل الطلب: معرف الطاولة أو معرفات العربة مفقودة.",
      Pleaseselectanewtabletotransfertheorder: "يرجى اختيار طاولة جديدة لنقل الطلب.",
      Failedtoidentifytheitemtoupdate: "فشل في تحديد العنصر للتحديث.",
      Missingrequireddatatoupdateitemstatus: "البيانات المطلوبة لتحديث حالة العنصر مفقودة.",
      StatuscannotbeupdatedviaAPIatthistime: "لا يمكن تحديث الحالة عبر API في هذا الوقت.",
      Failedtoapplythestatusupdatelocally: "فشل في تطبيق تحديث الحالة محليًا.",
      Failedtoupdatestatus: "فشل في تحديث الحالة.",
      PleasefillinallrequiredfieldsManagerIDandPassword: "يرجى ملء جميع الحقول المطلوبة: معرف المدير وكلمة المرور.",
      Itemvoidedsuccessfully: "تم إلغاء العنصر بنجاح!",
      InvalidManagerIDorPasswordAccessdenied: "معرف المدير أو كلمة المرور غير صالحة. تم رفض الوصول.",
      NoresponsefromserverCheckyourinternetconnection: "لا يوجد استجابة من الخادم. تحقق من اتصالك بالإنترنت.",
      Anunexpectederroroccurred: "حدث خطأ غير متوقع.",
      Pleaseselectitemstopayforfromthedoneitemslist: "يرجى اختيار العناصر للدفع من قائمة العناصر المكتملة.",
      PleaseselectitemschooseastatusandensureaTableIDisset: "يرجى اختيار العناصر، اختيار حالة، والتأكد من تعيين معرف الطاولة.",
      Novaliditemstoupdate: "لا توجد عناصر صالحة للتحديث.",
      BulkUpdateSuccess: "تم تحديث {{count}} عنصرًا بنجاح إلى {{status}}",
      Itemremovedsuccessfully: "تمت إزالة العنصر بنجاح",
      Noitemstoclear: "لا توجد عناصر للمسح.",
      OrderDetails: "تفاصيل الطلب",
      ClearAllItems: "مسح جميع العناصر",
      ViewOrders: "عرض الطلبات",
      ApplyOffer: "تطبيق العرض (النقاط)",
      ApplyDeal: "إتمام الطلب",
      ChangeTable: "تغيير الطاولة",
      ApplyStatus: "تطبيق الحالة",
      ConfirmRemoveAll: "هل أنت متأكد أنك تريد إزالة جميع العناصر وعددها {{count}}؟ هذا الإجراء لا يمكن التراجع عنه.",
      Cancel: "إلغاء",
      Preparation: "التحضير",
      Price: "السعر",
      Item: "العنصر",
      Quantity: "الكمية",
      Pay: "الدفع",
      Total: "المجموع",
      Void: "إلغاء",
      NoItemsFound: "لم يتم العثور على أي عناصر لهذا الطلب.",
      TotalOrderAmount: "المبلغ الإجمالي للطلب",
      EGP: "ج.م",
      SelectedItems: "العناصر المحددة ({{count}}):",

      AmountToPay: "المبلغ المطلوب دفعه :",
      Checkout: "الدفع",
      SaveasPending: "حفظ كمعلق",
      ApplyOfferUsePoints: "تطبيق العرض / استخدام النقاط",
      EnterLoyaltyOrRewardCode: "أدخل رمز الولاء الخاص بالعميل أو رمز عنصر المكافأة.",
      CheckCode: "تحقق من الرمز",
      ConfirmRewardPurchase: "تأكيد شراء المكافأة",
      UserID: "معرف المستخدم",
      ConfirmAddOffer: "تأكيد إضافة {{product}} إلى الطلب مقابل ({{points}} نقاط)؟",
      CustomerUserId: "العميل: رقم المستخدم **{{user_id}}**",
      ConfirmAddDeal: "تأكيد إضافة {{deal_title}} مقابل ({{deal_price}} جنيه) إلى الطلب؟",
      PendingOrders: "الطلبات المعلقة",
      OtherCharge: "رسوم أخرى",
      Tax: "ضريبة",
      SubTotal: "المجموع الفرعي",
      "Cart Has Items": "السلة تحتوي على منتجات",
      "You have": "لديك",
      "item(s) in your cart. What would you like to do?": "منتج(ات) في السلة. ماذا تود أن تفعل؟",
      "Clear & Switch": "مسح والانتقال",
      "Keep for Later": "الاحتفاظ بها",
      Notes: "ملاحظات",
      //item
      QuantityIncreased: "تم زيادة كمية \"{{name}}\" إلى {{quantity}}.",
      UpdateFailed: "فشل في تحديث \"{{name}}\".",
      ProductAdded: "\"{{name}}\" تمت إضافته بنجاح!",
      SubmitFailed: "فشل في تقديم \"{{name}}\".",
      ErrorLoadingData: "خطأ في تحميل البيانات. يرجى المحاولة مرة أخرى لاحقًا. تحقق من رمز المصادقة الخاص بك.",
      SelectBranchToViewItems: "يرجى اختيار فرع لعرض العناصر.",
      TakeAway: "تيك واي ",
      SelectCategory: "اختر الفئة",
      ByPiece: "بالقطعة",
      ByWeight: "بالوزن",
      SearchByProductName: "ابحث عن اسم المنتج...",
      Noproductsfoundfor: "لم يتم العثور على منتجات لـ",
      SelectedCategory: "الفئة المختارة",
      ShowMoreProducts: "عرض المزيد من المنتجات",
      // Cashier
      SelectionCashier: "اختيار  كاشير",
      Nocashiersavailable: "لا يوجد  كاشير متاح",
      ShowMore: "عرض المزيد",
      //shift
      ShiftOpenedSuccessfully: "تم فتح الوردية بنجاح! جاري إعادة التوجيه إلى الصفحة الرئيسية...",
      FailedToOpenShift: "فشل في فتح الوردية.",
      ShiftClosedSuccessfully: "تم إغلاق الوردية بنجاح!",
      FailedToCloseShift: "فشل في إغلاق الوردية",
      ShiftStatus: 'حالة الوردية',
      CurrentlyOnShift: "أنت حاليًا في الوردية",
      UpForShift: "أنت على وشك بدء الوردية",
      TakeYourShift: "ابدأ ورديتك",
      BackToWork: "العودة إلى العمل"
      , OpenNewShift: "فتح وردية جديدة",
      Redirectingtohomein: "جارٍ إعادة التوجيه إلى الصفحة الرئيسية في",
      SelectProduct: "اختر المنتج",
      Description: "الوصف",
      //productcard
      Adding: "جارٍ الإضافة...",
      AddToOrder: "أضف إلى الطلب",
      //Delivery
      AddUser: "إضافة مستخدم",
      Nousersfoundmatchingyoursearch: "لم يتم العثور على مستخدمين يطابقون بحثك",
      Noaddressavailable: "لا يوجد عنوان متاح",
      AddAddress: "إضافة عنوان",
      ConfirmDelivery: "تأكيد التوصيل",
      Pleasesearchbynameorphonetofindusers: "يرجى البحث بالاسم أو الهاتف للعثور على المستخدمين",
      Edituser: "تعديل المستخدم",
      FirstName: "الاسم الأول",
      LastName: "اسم العائلة",
      Save: "حفظ",
      Saving: "جارٍ الحفظ",
      AnotherPhone: "هاتف آخر",
      Phone: "الهاتف",
      SearchuserNameorPhone: "ابحث باسم المستخدم أو الهاتف",
      //Delivery
      Nodescriptionavailable: "لا يوجد وصف متاح",
      Min: "الحد الأدنى",
      Max: "الحد الأقصى",
      ExtrasOptional: "إضافات (اختياري)",
      Addons: "الإضافات",
      Variations: "الاختلافات",
      save: "Save",
      Free: "مجاني",
      AddonsOptional: "الإضافات (اختياري)",
      Weight: "الوزن",
      AddtoCart: "أضف إلى السلة",
      ExcludeOptional: "استبعاد (اختياري)",
      SpecialInstructionsOptional: "تعليمات خاصة (اختياري)",
      characters: "أحرف",
      AddSpecialInstructions: "أضف أي تعليمات خاصة لهذا العنصر...",
      Available: "متاح",
      Preorder: "طلب مسبق",
      Withorder: "مع الطلب",
      Reserved: "محجوز",
      CannotTransferToSameTable: "لا يمكن النقل إلى نفس الطاولة. يرجى اختيار طاولة مختلفة.",
      OrderTransferredSuccessfully: "تم نقل الطلب بنجاح!",
      FailedtotransfertablePleasetryagain: "فشل في نقل الطاولة. يرجى المحاولة مرة أخرى.",
      MergedTable: "طاولة مدمجة",
      Cap: "عدد",
      Source: "المصدر",
      Transferringorder: "جارٍ نقل الطلب...",
      Failedtoloadtables: "فشل في تحميل الطاولات",
      Pleasecheckyournetworkconnectionortryagainlater: "يرجى التحقق من اتصال الشبكة أو المحاولة مرة أخرى لاحقًا",
      SelectNewTableForTransfer: "اختر طاولة جديدة للنقل",
      DineInTables: "طاولات الصالة",
      NoTablesFound: "لم يتم العثور على طاولات لهذا الموقع.",
      SelectAnotherLocationOrAddTables: "يرجى اختيار موقع آخر أو إضافة طاولات.",
      NoCafeLocationsFound: "لم يتم العثور على مواقع مقاهي لهذا الفرع.",
      EnsureLocationsConfigured: "يرجى التأكد من تكوين المواقع في النظام.",
      SelectNewTableToTransferOrder: "يرجى اختيار طاولة جديدة لنقل الطلب.",
      SelectTableToTransferFrom: "اختر طاولة لنقل الطلب من الطاولة {{sourceTableId}}.",
      SelectTableToStartOrder: "اختر طاولة لبدء الطلب. الطاولات المدمجة مميزة باللون البنفسجي.",
      ConfirmClearAllItems: "تأكيد مسح جميع العناصر",
      //DeliveryInfo
      DeliveryInformation: "معلومات التوصيل",
      Failedtoloaduserinformation: "فشل في تحميل معلومات المستخدم ❌",
      CustomerName: "اسم العميل",
      PhoneNumber: "رقم الهاتف",
      DeliveryAddress: "عنوان التوصيل",
      Nouserinformationavailable: "لا توجد معلومات مستخدم متاحة ⚠️",
      Pleaseselectacustomerfromthedeliverypage: "يرجى اختيار عميل من صفحة التوصيل",
      //constants
      Pending: "معلق",
      Waiting: "انتظار",
      Preparing: "جارٍ التحضير",
      PickUp: "استلام",
      Done: "تم",
      ApproveandAddDeal: "الموافقة وإضافة الصفقة",
      //ordersview
      Statusupdatedsuccessfully: "تم تحديث الحالة بنجاح",
      Errorupdatingstatus: "خطأ في تحديث الحالة",
      Orders: "الطلبات:",
      SearchOrderNumber: "ابحث برقم الطلب",
      NoOrdersFound: "لم يتم العثور على طلبات",
      Updating: "جارٍ التحديث...",
      DineIn: "صالة",
      Items: "العناصر",
      ApplyDealCode: "تطبيق رمز الصفقة",
      CheckDeal: "تحقق من الصفقة",
      ConfirmDealAcceptance: "تأكيد قبول الصفقة",
      EnterDealCode: "أدخل رمز الصفقة",
      EnterOfferCode: "أدخل رمز العرض",
      ApproveandAddOffer: "الموافقة وإضافة العرض",
      tak_away: "استلام شخصي",
      delivery: "توصيل",
      //DeliveryAdd
      Pleaseselectacity: "يرجى اختيار مدينة",
      Pleaseselectazone: "يرجى اختيار منطقة",
      Pleaseprovideavalidaddress: "يرجى تقديم عنوان صالح",
      AddressIDnotfound: "معرف العنوان غير موجود",
      Addressupdated: "تم تحديث العنوان",
      Addressadded: "تمت إضافة العنوان",
      Useradded: "تمت إضافة المستخدم",
      successfully: "بنجاح",
      //DeliveryAdd
      Operationfailed: "فشل العملية",
      FailedtosubmitPleasetryagain: "فشل في الإرسال. يرجى المحاولة مرة أخرى.",
      Unknownerror: "خطأ غير معروف",
      AddAnotherAddressforUser
        : "إضافة عنوان آخر للمستخدم",
      AddNewDeliveryUser: "إضافة مستخدم توصيل جديد",
      EditDeliveryAddress: "تعديل عنوان التوصيل",
      AddressSelectionMode: "وضع اختيار العنوان",
      Automaticfrommaplocation: "تلقائي من موقع الخريطة",
      Manualaddressentry: "إدخال العنوان يدويًا",
      Manual: "يدوي",
      fromMap: "(من الخريطة)",
      Auto: "تلقائي",
      ManualEntry: "(إدخال يدوي)",
      Clickotaddress: "انقر على الخريطة لتحديد العنوان",
      Enteryouraddressmanually: "أدخل عنوانك يدويًا",
      Street: "الشارع",
      BuildingNumber: "رقم المبنى",
      FloorNumber: "رقم الطابق",
      Apartment: "الشقة",
      Landmark: "معلم بارز",
      AdditionalNotes: "ملاحظات إضافية",
      SelectType: "اختر النوع",
      Home: "المنزل",
      Work: "العمل",
      Other: "أخرى",
      AdditionalData: "بيانات إضافية",
      Zone: "المنطقة",
      City: "المدينة",
      SelectZone: "اختر المنطقة",
      SelectCity: "اختر المدينة",
      Address: "العناوين",
      UpdateAddress: "تحديث العنوان",
      Errorloadingpendingorders: "خطأ في تحميل الطلبات المعلقة",
      Errorloadingorderdetails: "خطأ في تحميل تفاصيل الطلب",
      Retry: "إعادة المحاولة",
      Nopendingordersfound: "لم يتم العثور على طلبات معلقة",
      Loadingorderdetails: "جارٍ تحميل تفاصيل الطلب",
      EditProfile: "تعديل الملف الشخصي",
      Imagesizemustbelessthan2MB: "يجب أن يكون حجم الصورة أقل من 2 ميجابايت",
      Usernameisrequired: "اسم المستخدم مطلوب",
      Passwordmustbeatleast3characters: "يجب أن تكون كلمة المرور 3 أحرف على الأقل",
      Profileupdatedsuccessfully: "تم تحديث الملف الشخصي بنجاح",
      Inactive: "غير نشط",
      Active: "نشط",
      EndShiftReport: "تقرير نهاية الشيفت",
      Employee: "الموظف",
      ShiftDuration: "مدة الشيفت",
      FinancialSummary: "الملخص المالي",
      TotalCashInShift: "إجمالي النقدية في الشيفت",
      OrdersSummaryByType: "ملخص الطلبات حسب النوع",
      PaymentMethodsBreakdown: "تفاصيل طرق الدفع",
      TotalAllOrders: "إجمالي كل الطلبات",
      Expenses: "المصروفات",
      TotalExpenses: "إجمالي المصروفات",
      NetCashInDrawer: "صافي النقدية المتبقية",
      TotalSales: "إجمالي المبيعات",
      NetCash: "صافي النقدية",
      ConfirmCloseShift: "تأكيد إغلاق الشيفت",
      Unknown: "غير معروف",
      ConfirmShiftClosure: "تأكيد إغلاق الشيفت",
      EnterPasswordToCloseShift: "أدخل كلمة المرور لإغلاق الشيفت",
      Confirm: "تأكيد",
      Apply: "تطبيق",
      Cancel: "إلغاء",
      ChangeTable: "تغيير الطاولة",
      AreYouSureTransferItems: "هل تريد نقل العناصر المختارة إلى طاولة أخرى؟",
      ConfirmTransfer: "تأكيد عملية النقل",
      ConfirmOrderWithDueModule: "تأكيد الطلب مع Due Module ({{amount}} ج.م)",
      PlatformWillPayRemaining: "المنصة ستدفع الباقي",
      ChooseStatus: "اختر حالة",
      PleaseEnterModuleOrderNumber: "يرجى إدخال رقم الطلب",
      EnterModuleOrderNumber: "أدخل رقم طلب الموديول",
      ModuleOrderNumber: "رقم الطلب",
      Save: "حفظ",
      UnknownProduct: "منتج غير معروف",
      UnknownAddon: "إضافة غير معروفة",
      ConfirmOrder: "تأكيد الطلب",
      CashInHandAmount: "كم المبلغ الموجود في العهدة الآن؟",
      ConfirmAndSend: "تأكيد وإرسال",
      Loading: "جاري التحميل...",
      SelectCustomerForDueOrder: "اختر عميل للطلب الآجل",
      OrderTotal: "إجمالي الطلب",
      DebitLimit: "حد المديونية",
      LimitExceeded: "تجاوز الحد",
      NoCustomersFoundRefineSearch: "لم يتم العثور على عملاء. يرجى تحسين البحث.",
      Searching: "جارٍ البحث...",
      OrderExceedsDebitLimitWithValues: "مبلغ الطلب ({{amount}} ج.م) يتجاوز حد المديونية للعميل ({{limit}} ج.م).",
      OrdersModeActivated: "تم تفعيل وضع الطلبات",
      FailedToLoadOrders: "فشل جلب الطلبات",
      PleaseSelectDateRange: "يرجى تحديد نطاق التاريخ",
      ThankYouForVisit: "شكراً لزيارتكم",
      DeliveryFees: "رسوم التوصيل",
      CustomerInfo: "بيانات العميل",
      NoOrdersFoundClickSearch: "لا توجد طلبات - اضغط بحث بعد تحديد التواريخ",
      NoOrdersMatchFilters: "لا توجد طلبات مطابقة للفلاتر الحالية",
      FailedToRefreshOrders: "فشل تحديث الطلبات",
      More: "المزيد",
      New: "جديد",
      User: "المستخدم",
      UserAccount: "حساب المستخدم",
      Profile: "الملف الشخصي",
      NormalPrices: "الأسعار العادية",
      RegularMenuPrices: "الأسعار العادية",
      NewOrderReceived: "تم استلام طلب جديد!",


    },
  },
};
i18n.use(initReactI18next).init({
  resources,
  lng: localStorage.getItem("language") || "en",
  fallbackLng: "en",
  interpolation: {
    escapeValue: false,
  },
});


export default i18n;
